...
| 310 Result    | This can be done from the review section on the product detail page on each product. A registered user can only delete their own review if they have already left one on a specific product. Click 'Delete' button on the review. Before they review is deleted, the user is met with an on-screen popup, asking if they are sure. Confirming this will delete their product review. |
